...
void uart1_init(ulong baudrate)
{
	...
	LL_UART_Init(UART1,&init);

	__LL_UART_TxFIFO_Reset(UART1);
	__LL_UART_TxDoneIntPnd_Clr(UART1);
	__LL_UART_Tx_En(UART1);
	__LL_UART_Tx_En(UART1);

	__LL_UART_RxFIFO_Reset(UART1);
	__LL_UART_Rx_En(UART1);

	ring_buff_init(&txbuff1);
}
...
{
	ring_buff_save_data(&txbuff1,data,size);
}
...
void uart1_tx_poll()
{
	...
	//如果数据未发送完成
//	if(!__LL_UART_IsTxDoneIntPnd(UART1))return;
//	__LL_UART_TxDoneIntPnd_Clr(UART1);

	//如果TxFIFO已满，则退出
	if(__LL_UART_IsTxFIFOFull(UART1))return;

	if(!ring_buff_take_byte(&txbuff1,&byte))return;
...
{
	if(__LL_UART_IsRxFIFOEmpty(UART1))return false;
	*data=(uchar)(__LL_UART_RxDat8bits_Read(UART1));
	return true;
}
...
{
	return ring_buff_is_empty(&txbuff1);
}
