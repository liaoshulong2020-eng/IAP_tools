...
#define PFC_MAX_DUTY 										(PFC_DRIVER_PERIOD_VALUE*0.95)
...
#define ACN_SAMPLE_ADC0_CH1   						ADC_CH_1			//ADC1_IN1 ==> PA0	交流N线电压采样信号
...
//  <i>范围 0~300V
//  <0-300>
#define	VAC_IN_UNDER_VOLTAGE								141.0

// <o>pfc最小输入电压(欠压恢复)
//  <i>范围 0~300V
//  <0-300>
#define	VAC_IN_ON_VOLTAGE		  							148.0

// <o>pfc输入过压电压
//  <i>范围 0~300V
//  <0-300>
#define	VAC_IN_OVER_VOLTAGE 	  						282.0

// <o>pfc输入最大电压(过压恢复)
//  <i>范围 0~300V
//  <0-300>
#define	VAC_IN_MAX_VOLTAGE 	 								275.0
...
//<i>范围 0~500V
//  <0-500>
#define	VOUT_VOLTAGE  				415 //200//

// <o>pfc输出欠压电压
//  <i>范围 0~500V
//  <0-500>
#define	VOUT_UNDER_VOLTAGE		345 //160//
...
#define ILOOP_INTEGRA_MIN_FORWD		(-0.82)
...
typedef struct
{
  ...
  uint32_t ac_voltage;					//用于正负半周环路运算
  uint32_t vin_squared;		      //输入电压平方
  uint32_t ac_voltage_avg;			//用于Cal the vin ac avg
  uint32_t ac_voltage_rms;		  //输入电压平方

  uint32_t rated_power;					//定功率


  bool positive_period;						//正周期
  uint32_t positive_cnt;		//正周期计数
  uint32_t positive_squares_sum;		//正半周期平方和
  uint32_t positive_average_sum;
  uint32_t		ac_drop_count;
  bool ac_drop;

  bool negative_period;						//负周期
  uint32_t negative_cnt;		//负周期计数
  uint32_t negative_squares_sum;		//负半周期平方和
  uint32_t	negative_average_sum;

  uint32_t  vin_squares_average;//平方和的平均值
  uint32_t  vin_squares_average_delta;
  uint32_t	vin_squared_slow_average;
  uint32_t vin_squared_slow_average_filterd;
  uint32_t	vin_squared_for_ac_drop;
  uint32_t	vin_squares_average_2;//平方和的平均值的平方
...
//		uint32_t 	ac_current_deltadelta;
  ...
  uint32_t 	ibus_target;						//目标电流
  uint32_t 	ibus_target_average;		//平均电流
  uint32_t 	ibus_samp_rel;					//当前瞬时电流
  uint32_t	ibus_target_offset;
  uint32_t  vff_multiplier;
  uint32_t  ibus_LPF;
  uint32_t  ibus_filtered;
  uint32_t	ibus_cal_avg;
  uint32_t  temp_filtered;

  uint32_t	vac_voltage;
  uint32_t 	vin_rel;
  uint32_t 	vin_sum;
  uint32_t 	vin_filtered;
  uint32_t  vin_filtered_last;
  uint32_t  vin_filtered1;
  uint32_t  vin_filtered_max;

  uint32_t	move_vin_sum;
  uint32_t	move_calcu_vin[200];
  uint32_t	move_vin_average;
