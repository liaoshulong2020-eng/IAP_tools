...
namespace IAPManager
{
    public partial class MainForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            cmbChipType.Items.AddRange(new object[] { "5800D", "5800C", "5300" });
        ...
        private void CmbChipType_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbChipType.SelectedItem != null)
            {
                ...
                else if (chipType == "5800C" || chipType == "5800")
                {
                    txtProjectName.Text = target == "PFC" ? "227B-PFC" : "227B-LLC";
                }
  ...
  ""readWriteSize"": {readWriteSize}
        ...
        */
        ...
        private void BtnUserConfig_Click(object sender, EventArgs e) => ExecuteUserConfig(showMsg: true);

        private bool ExecuteUserConfig(bool showMsg)
        {
            if (string.IsNullOrEmpty(projectPath))
            {
                if (showMsg) MessageBox.Show("请先选择工程文件夹！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }

            if (string.IsNullOrEmpty(projectName))
            {
                if (showMsg) MessageBox.Show("请输入项目名称！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }

            if (!rbOnlineMode.Checked && !rbOfflineMode.Checked)
            {
                if (showMsg) MessageBox.Show("请先选择升级模式！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }

            try
            {
                LogMessage("=== [1/4] 开始应用配置 ===");
                ...
                if (!File.Exists(scatterPath))
                {
                    ...
                    string otherChipType = chipType == "5300" ? "5800C" : "5300";
                    string otherScatterPath = Path.Combine(projectPath, GetScatterFileName(otherChipType));
                    if (File.Exists(otherScatterPath))
                    {
                        if (showMsg) MessageBox.Show($"当前目录包含{otherChipType}芯片的项目文件！\n\n请：\n1. 切换芯片类型为{otherChipType}，或\n2. 选择正确的{chipType}芯片项目目录",
                            "芯片类型不匹配", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                    {
                        if (showMsg) MessageBox.Show($"找不到{chipType}芯片的Scatter文件！\n请确保选择了正确的项目目录。",
                ...
                if (!ValidateScatterFileFormat(content))
                {
                    ...
                    if (showMsg) MessageBox.Show("Scatter文件格式不正确！请检查第一行是否为ARM预处理器指令。", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
            ...
            catch (Exception ex)
            {
                ...
                lblStatus.ForeColor = System.Drawing.Color.Red;
                if (showMsg) MessageBox.Show($"配置失败: {ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
  ...
  ""appMaxSize"": {config.appMaxSize}
        ...
        */
        ...
        private void BtnCreatePowerShellScript_Click(object sender, EventArgs e) => ExecuteCreatePowerShellScript(showMsg: true);

        private bool ExecuteCreatePowerShellScript(bool showMsg)
        {
            if (string.IsNullOrEmpty(projectPath))
            {
                if (showMsg) MessageBox.Show("请先选择工程文件夹！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            try
            {
                LogMessage($"=== [4/4] 开始创建嵌入的PowerShell脚本 (芯片: {chipType}) ===");
                ...
                LogMessage($"=== [4/4] PowerShell脚本创建完成 (芯片: {chipType}) ===");
                lblStatus.Text = $"状态: PowerShell脚本已创建 ({chipType}芯片)";
                lblStatus.ForeColor = System.Drawing.Color.Green;

                if (showMsg) MessageBox.Show($"PowerShell脚本创建成功！\n芯片类型: {chipType}", "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return true;
            }
            catch (Exception ex)
            {
                ...
                lblStatus.ForeColor = System.Drawing.Color.Red;
                if (showMsg) MessageBox.Show($"创建PowerShell脚本失败: {ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }

        private void BtnCreateBootLoaderFiles_Click(object sender, EventArgs e) => ExecuteCreateBootLoaderFiles(showMsg: true);

        private bool ExecuteCreateBootLoaderFiles(bool showMsg)
        {
            if (string.IsNullOrEmpty(projectPath))
            {
                if (showMsg) MessageBox.Show("请先选择工程文件夹！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            {
                string target = cmbTarget?.SelectedItem?.ToString() ?? "LLC";
                LogMessage($"=== [3/4] 开始创建嵌入的BootLoader文件 (芯片类型: {chipType}, 目标: {target}) ===");
                ...
                LogMessage($"=== [3/4] {chipType}芯片BootLoader文件创建完成 ===");
                lblStatus.Text = $"状态: {chipType}芯片BootLoader文件已创建";
                lblStatus.ForeColor = System.Drawing.Color.Green;

                if (showMsg) MessageBox.Show($"{chipType}芯片BootLoader文件创建成功！", "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return true;
            }
            catch (Exception ex)
            {
                ...
                lblStatus.ForeColor = System.Drawing.Color.Red;
                if (showMsg) MessageBox.Show($"创建{chipType}芯片BootLoader文件失败: {ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
        ...
        private void BtnQuickFix_Click(object sender, EventArgs e) => ExecuteQuickFix(showMsg: true);

        private bool ExecuteQuickFix(bool showMsg)
        {
            if (string.IsNullOrEmpty(projectPath))
            {
                if (showMsg) MessageBox.Show("请先选择工程文件夹！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            try
            {
                LogMessage($"=== [2/4] 开始快速修复{chipType}芯片scatter文件格式 ===");
                ...
                {
                    LogMessage($"错误: {chipType}芯片Scatter文件不存在: {scatterPath}");
                    if (showMsg) MessageBox.Show($"{chipType}芯片Scatter文件不存在！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                ...
                LogMessage($"=== [2/4] {chipType}芯片快速修复完成 ===");
                lblStatus.Text = $"状态: {chipType}芯片快速修复完成";
                lblStatus.ForeColor = System.Drawing.Color.Green;

                LoadCurrentConfiguration();

                if (showMsg) MessageBox.Show($"{chipType}芯片快速修复完成！\n已将分号注释转换为块注释格式。",
            ...
            catch (Exception ex)
            {
                ...
                lblStatus.ForeColor = System.Drawing.Color.Red;
                if (showMsg) MessageBox.Show($"快速修复失败: {ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
