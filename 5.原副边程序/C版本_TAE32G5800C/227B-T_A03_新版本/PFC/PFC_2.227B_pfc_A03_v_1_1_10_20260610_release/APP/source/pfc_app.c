...
//  pfc.vin_filtered = pfc.vin_sum >> 3;
  pfc.vin_filtered = (uint32_t)pfc.vin_voltage;
  pfc.vdc_filtered = (uint32_t)pfc.vbus_value + pfc.vdc_filtered - (pfc.vdc_filtered >> VBUS_FILTER_CNT);
  pfc.vdc_bus_square = ((pfc.vdc_filtered >> VBUS_FILTER_CNT) * (pfc.vdc_filtered >> VBUS_FILTER_CNT)) >> 11;
}

RAMCODE
void calculate_cur_target(void)
{
  pfc.ac_current_ref = ((pfc.ibus_target_average * pfc.vin_filtered) >> 16) + pfc.ibus_target_offset ;

  if(pfc.ac_current_ref > 8192)
    {
      pfc.ac_current_ref = 8192;
    }
  else if(pfc.ac_current_ref <= 0)
    {
      pfc.ac_current_ref = 0;
    }
}
...
void half_cycle_processing(void)
{
  ...
  pfc.vin_rel = (uint32_t)(FastInverseSqrt((float)pfc.move_vin_average));
...
void K_forward_calcula()
{
  ...
  if(pfc.vbus_rel < 200)
    {
      pfc.k_forward = 0.8 * (1 - (float)((pfc.vin_voltage) / (200))) ;
    }
  else
    {
      pfc.k_forward = 0.8 * (1 - (float)((pfc.vin_voltage) / (pfc.vbus_rel)));
    }
  if(pfc.k_forward > (0.8))
    {
      pfc.k_forward = (0.8);
    }
}
...
//  uint32_t delta_v, vin_square_temp;
  if(pfc.r_ntc_value < 200 && pfc.is_ac_ok )
    {
...
{
  if(pfc.is_no_fault && pfc.pre_finish_flg && pfc.start_cnt_flg)
//        if(pfc.zero_point == NEGA_TO_POSI)							//过零点才启动
    {
      ...
      if(pfc.vbus_ref >= pfc.vbus_target)	//参考值达到设定的目标值
        ...
        {
          pfc.vbus_ref = pfc.vbus_ref + 0.02;//4		//5							//未达到设定的目标值则继续增加
        }
    }
  else
    {
