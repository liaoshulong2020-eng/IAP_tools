...
void K_forward_calcula()
{
  ...
  if(pfc.vbus_rel < 200)
    {
      pfc.k_forward = 0.8f * (1.0f - (float)((pfc.vin_voltage) / (200.0f))) ;
    }
  else
    {
//			if(pfc.vin_voltage < 80.0f)
//			{
//				pfc.k_forward=0.5564f*((pfc.vin_voltage/80.0f)*(pfc.vin_voltage/80.0f));// 0.64 = 0.8*(1-80/394)
////				pfc.k_forward = 0.8f * (1.0f - (float)((pfc.vin_voltage) / (pfc.vbus_rel)));
//			}
//			else
			{
				pfc.k_forward = 0.8f * (1.0f - (float)((pfc.vin_voltage) / (pfc.vbus_rel)));
...
//  uint32_t delta_v, vin_square_temp;
  if(pfc.r_ntc_value < 300 && pfc.is_ac_ok)
    {
//			hrpwm_pre_outen();
...
//
#if 1 //输入欠压补丁
	if(pfc.is_llc_rampup_cnt < 10000)
	{
		pfc.is_llc_rampup_cnt++;
		pfc.is_llc_rampup = 1;
	}
	else
	{
		pfc.is_llc_rampup = 0;
		pfc.is_llc_rampup_cnt = 10001;
	}
#else
			pfc.is_llc_rampup = 0;
#endif
