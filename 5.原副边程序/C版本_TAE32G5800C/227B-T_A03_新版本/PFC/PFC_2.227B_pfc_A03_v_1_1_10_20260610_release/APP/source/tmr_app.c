...
//			SPLL_Update(&spll, pfc.vin_voltage); // Update SPLL with new sample
      ...
      tx_vofa_data.f[4] = pfc.k_forward;//check_current_is_ok;//(float)pfc.duty*100/PFC_DRIVER_PERIOD_VALUE;;//pfc.vbus_target;//(2 * 3.14159 * 2608 * 50 * 4);
      tx_vofa_data.f[5] =	HRPWM->Common.OENR;
			tx_vofa_data.f[6] =	pfc.r_ntc_value;
			tx_vofa_data.f[7] =	pfc.vin_rel ;
			tx_vofa_data.f[8] =	LL_GPIO_ReadPin(GPIOA,GPIO_PIN_8);
      user_vofa_tx();
