...
void pfc_PI_iloop()
{
  ...
  else if(pfc.iloop.integral < (pfc.iloop.integral_min))
    {
      pfc.iloop.integral = pfc.iloop.integral_min;
		...
		pfc.iloop.loop_out = (pfc.iloop.p_out + pfc.iloop.integral) / 4;
  ...
  else if(pfc.iloop.integral < (pfc.iloop.integral_min))//- 1<<8
    {
      pfc.iloop.integral = pfc.iloop.integral_min;//
		...
		pfc.iloop.loop_out = (pfc.iloop.p_out + pfc.iloop.integral) / 8 ;  
...
void pfc_PI_vloop(void)
{
  ...
  float  i_mul = 0, i_out = 0; //p_out,
	
	if(pfc.vloop.out_max < VLOOP_OUTPUT_MAX)
		{
			  pfc.vloop.out_max+= 0.2;
		} 
		else
		{
...
void pfc_iloop_init()
{
	pfc.iloop.k1 = 0.01155;
  pfc.iloop.k2 = -0.00845;
		
	pfc.iloop.kp = 0.07;//0.1;//0.12;
  pfc.iloop.ki = 0.0040;//0.01;			
	pfc.iloop.integral = 0;
	pfc.iloop.p_out = 0;
