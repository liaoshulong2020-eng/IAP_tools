...
static bool app_vector_is_valid(){ulong sp=*((ulong*)APP_BASE_ADDR),reset=*((ulong*)(APP_BASE_ADDR+4)),ra=reset&(~1UL);if(sp<RAM_BASE_ADDR||sp>RAM_END_ADDR||(sp&3UL)!=0)return false;if((reset&1UL)==0)return false;return ra>=APP_BASE_ADDR&&ra<APP_END_ADDR;}
...
{
	if(!iap_flag || pkt->len>IAP_DATA_MAX || !range_is_valid(pkt->addr,pkt->len,APP_BASE_ADDR,APP_END_ADDR)){pkt->len=0;pkt->size=0;return;}
...
static void cmd_write_checksum(iap_pkt_t *pkt)
{
	...
	if(appsize==0 || appsize>APP_MAX_SIZE || appsize>(APP_END_ADDR-APP_BASE_ADDR)){pkt->len=0;pkt->size=0;return;}
