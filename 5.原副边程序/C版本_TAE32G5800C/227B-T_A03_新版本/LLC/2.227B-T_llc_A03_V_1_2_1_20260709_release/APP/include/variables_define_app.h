...
#define	LLC_SW_FREQ_FR					(199)	//199k
#define LLC_SW_PERIOD_FR				(uint32_t)(HRPWM_FINAL_FREQ/LLC_SW_FREQ_FR)//register value

#define	LLC_SW_FREQ_MIN					(155) // 150k
#define LLC_SW_PERIOD_MAX				(uint32_t)(HRPWM_FINAL_FREQ/LLC_SW_FREQ_MIN)//register value

#define LLC_DEADTIME						(float)(150)//LLC驱动上下管死区
#define LLC_DEADTIME_COUNT			((uint32_t)(LLC_DEADTIME/HRPWM_ONE_STEP_TIME))	//	432

#define LLC_DRIVER_DEADTIME			(float)	(30)
#define LLC_DRIVER_DEADTIME_COUNT	((uint32_t)(LLC_DRIVER_DEADTIME/HRPWM_ONE_STEP_TIME))	

#define LLC_DUTY_MIN_COUNT			((LLC_DEADTIME_COUNT+1500)*2) //	3864
#define LLC_DUTY_MAX_COUNT			((LLC_SW_PERIOD_MIN/2)-2*LLC_DEADTIME_COUNT) //3682

#define LLC_DUTY_BURST_COUNT			((LLC_SW_PERIOD_MIN/2)-LLC_DEADTIME_COUNT) //

#define LLC_BURST_DUTY_MAX_COUNT	((LLC_DEADTIME_COUNT+100)*2) 	//1064
#define LLC_BURST_DUTY_COUNT		((LLC_SW_PERIOD_MIN/2 - 2*LLC_DEADTIME_COUNT))  //3682
/*================================================HRPWM================================================*/
...
#define TEMP_SAMPLE_CH1              	  ADC_CH_1			//主板铝基板温度采样信号，保护用
#define S_TRIM_SAMPLE_CH2               ADC_CH_2			//LLC输出电压远端补偿信号
#define VOUT_SAMP_CH4										ADC_CH_4		//输出电压采样

/*============================ ADC1 ============================*/

#define VFB_SAMPLE_CH4               		ADC_CH_4  		//LLC输出电压采样信号(主)
#define LOADSHARE_SAMPLE_CH12           ADC_CH_12  	//系统均流总线电压采样
#define ADDR1_SAMPLE_CH13              	ADC_CH_13	  //ADDR1采样信号
#define ADDR3_SAMPLE_CH3              	ADC_CH_3	  //ADDR3采样信号



/*============================ ADC2 ============================*/
#define IOUT_SAMPLE_CH12              	ADC_CH_12	  //LLC输出电流采样信号
#define ADDR2_SAMPLE_CH1             		ADC_CH_1 		 //ADDR2采样
...
#define IOUT_FILTER_ORDER				4//2^3
#define TEMP_FILTER_CNT					16//2^44
#define TRIM_FILTER_CNT					128//2^6
#define LOADSHARE_FILTER_CNT		4//2^15
#define ADDR1_FILTER_CNT				4
#define IOUT_SLOW_FILTER_CNT		64
#define VBUS_SLOW_FILTER_CNT		8
/*--------------------ADC VREF Define--------------------*/
#define	SAMP_RATIO												(float)(2.9/8192)		//采样比例

#define	RATED_POUT_WATT				((float)(1200)) //额定输出功率
#define	MAX_POUT_WATT					((float)(1400)) //最大输出功率	
#define	SAFE_POUT_WATT				((float)(1300)) //降额输出功率
	
#define	DOWM_POUT_WATT				((float)(600)) //下调降额输出功率
#define	DOWM_OCP_WATT				((float)(700)) //下调降额过流点功率
...
//  <i>范围 10V~14V
//  <10-14>
#define VOUT_VOLTAGE					((float)(28.0))
	
#define VOUT_UP_VOLTAGE				((float)(28.5))
	
#define VOUT_DOWM_VOLTAGE			((float)(27.2))

// <o>输出欠压(软件保护)
//  <i>范围 9V~11V
//  <9-11>
#define VOUT_UNDER_VOLTAGE			((float)(22.0))
// <o>输出过压(软件保护)
//  <i>范围 13V~16V
//  <13-16>
#define VOUT_OVER_VOLTAGE				((float)(33.0))

#define VOUT_GAIN								((float)0.073732)
...
#define IOUT_TARGET_CURRENT			(float)(50.0+5.0)//104//12//86
#define IOUT_REC_TARGET_CURRENT	(float)(46.0)//104//12//86

#define IOUT_MAX_CURRENT				(RATED_POUT_WATT/VOUT_VOLTAGE)	//42.857
#define IOUT_OCP_CURRENT				(float)(52.0+5.0)//104//15//90
#define IOUT_SHORT_CURRENT			(float)(54.0+5.0)//105//18//92

#define IOUT_SR_ON_CURRENT			5//20//8
#define IOUT_SR_OFF_CURRENT			2//6//3

#define IOUT_GAIN_VAL						((float)0.0155) //((float)0.0205)  
#define IOUT_OFFSET_VAL					((float)0.2913)	
	
// 采样系统转换系数
#define IOUT_SAMPLING_COEFF_A		(IOUT_GAIN_VAL * 8192 / 2.9)
#define IOUT_SAMPLING_COEFF_B		(IOUT_OFFSET_VAL * 8192 / 2.9)

#define	IOUT_OFFSET_VAL_CHECK		(float)(-4.3865)
#define IOUT_GAIN_VAL_CHECK			(float)(1.0549)
...
//  <i>范围 13V~16V
//  <13-16>
#define VOUT_OVER_VOLTAGE_DAC			33.0
#define VOUT_OVER_VOLTAGE_DAC_VALUE				(VOUT_OVER_VOLTAGE_DAC*VOUT_GAIN*4096/2.9)

// <o>输出过流(硬件保护)
//  <i>范围 10A-90A
//  <10-90>
#define IOUT_OCP_CURRENT_DAC									(70.0f)
#define IOUT_OCP_CURRENT_DAC_DAC_VALUE				((IOUT_OCP_CURRENT_DAC*IOUT_GAIN_VAL+IOUT_OFFSET_VAL)*4096/2.9)

#define IOUT_REC_CURRENT_DAC									(50.0f)
#define IOUT_REC_CURRENT_DAC_DAC_VALUE				((IOUT_REC_CURRENT_DAC*IOUT_GAIN_VAL+IOUT_OFFSET_VAL)*4096/2.9)
...
typedef enum
{
    ...
    CMD_PARA_SCALE = 0x27,			// 参数倍数（32位，带CRC）
    CMD_KP = 0x28,              // 设置KP
    CMD_KI = 0x29,              // 设置KI
...
//日期定义 20250815
#define 	YEAR_NUM							25		//byte3
#define 	MONTH_NUM							8	 		//byte4
#define 	DAY_NUM								15 		//byte5
...
#define 	HLD_MONTH_NUM							7	 	//byte4
#define 	HLD_DAY_NUM								9 		//byte5
#define 	HLD_MAJOR_VERSION					1
#define 	HLD_MINOR_VERSION					2
#define 	HLD_PATCH_VERSION					1
...
typedef struct {
    ...
    float 								para_scale_factor;        // 参数缩放倍数
    bool									para_scale_factor_is_ok	;	//参数缩放倍数设定ok
} can_data_TypeDef;
...
#define	SHARE_OUT_MIN						(float)(-2.0f)
#define	SHARE_OUT_MAX						(float)(2.0f)
...
typedef union {
    struct {
        uint8_t can1_level : 1;  // 表示第一个电平的高低
        uint8_t can2_level : 1;  // 表示第二个电平的高低
        uint8_t can3_level : 1;  // 表示第三个电平的高低
        uint8_t reserved : 5; // 保留位，确保总共占用 8 位
    };
    uint8_t can_addr_level_bits;  // 使用一个字节来存储所有电平的组合
} Can_LevelUnion;
...
float shareloop_ref_flt;
uint16_t addr1_value;
uint32_t addr1_value_store;
uint16_t addr2_value;
uint32_t addr2_value_store;
uint16_t addr3_value;
uint32_t addr3_value_store;
...
uint32_t pr_open_cnt;
uint32_t pr_close_cnt;
...
volatile float pr_control_output;
	...
	float vout_hw_trim_delta;
	TRIM_DIRECT_TypeDef trim_direct;
	
uint32_t	test_cnt;
uint32_t burst_cnt_temp;
...
bool 		 check_ovp_flag;
float 		para_scale_factor;
bool 			para_scale_factor_is_ok;
bool			protec_set_flag;
} llcTypeDef;
