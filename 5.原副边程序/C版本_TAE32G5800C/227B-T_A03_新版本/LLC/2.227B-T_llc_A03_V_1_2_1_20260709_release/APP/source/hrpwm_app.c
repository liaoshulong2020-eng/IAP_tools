...
//  LL_HRPWM_Comm_FltCfg(HRPWM, HRPWM_FLT_NUM_0, &user_hrpwm_comm_flt_cfg);
  ...
  user_hrpwm_comm_flt_cfg.src      = HRPWM_COMM_FLT1_INPUT_SRC_CMP3_OUT;
  LL_HRPWM_Comm_FltCfg(HRPWM, HRPWM_FLT_NUM_1, &user_hrpwm_comm_flt_cfg);

//  __LL_HRPWM_Slv_Flt0_En(HRPWM, LLC_DRIVER_SLAVE_2);
//  __LL_HRPWM_Slv_Flt0_En(HRPWM, SR_DRIVER_SLAVE_3);

//	__LL_HRPWM_Slv_Flt1_En(HRPWM, LLC_DRIVER_SLAVE_2);
//	__LL_HRPWM_Slv_Flt1_En(HRPWM, SR_DRIVER_SLAVE_3);
...
//				}
		...
		else
		{
					pwm3.period = llc.period;
					pwm3.compa  = LLC_DRIVER_DEADTIME_COUNT;
					pwm3.compb  = (llc.period / 2) - LLC_DEADTIME_COUNT-200;
					pwm3.compc  = (llc.period / 2) + LLC_DRIVER_DEADTIME_COUNT;
					pwm3.compd  = llc.period - LLC_DEADTIME_COUNT-200;
		}	
		#endif
...
// if(llc.switch_mode == burst_mode || llc.mode_hd)
  ...
  if((llc.sr_pwm) && (llc.switch_mode == pfm_mode || llc.switch_mode == max_pfm_mode))
    {
...
//      else
//        {
//          llc.loop_out = (llc.loop_out - llc.loop_out/5);
//          llc_pwm_value_ctrl();
