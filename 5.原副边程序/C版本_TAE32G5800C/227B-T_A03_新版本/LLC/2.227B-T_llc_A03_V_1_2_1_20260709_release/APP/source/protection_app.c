...
void under_output_voltage_check_app(void)		//输出欠压
{
  if(llc.state == state_on)									//完成稳压后才开始检测欠压
    {
      llc.state_on_cnt++;
      if((llc.vbus_rel < llc.protection_point.under_voltage_point) && (llc.state_on_cnt > 10))
        {
...
void over_current_check_app(void)
{
  if(!llc.fault_state.bit.over_current)
    {
//      if(llc.iout_rel > IOUT_OCP_CURRENT)
				if(llc.iout_rel > llc.protection_point.over_current_point)
        {
          ...
          if((llc.fault_cnt.over_current_cnt > 1) && (llc.fault_cnt.over_current_cnt < 10))
            {
      ...
      if(llc.fault_cnt.over_current_cnt > 100)
        {
//					if(llc.state != state_idel)
//					{
//						llc.test_tx |=(1<<9);
//						llc.state = state_fault;
//					}
//          if(llc.fault_state.bit.under_voltage == 0)
          {
            llc.fault_state.bit.over_current = 1 ;//	过流
//						LL_DAC_ValueSet(DAC0,IOUT_REC_CURRENT_DAC_DAC_VALUE);
						LL_CMP_Start(CMP3);
//            GPIOB->DOR ^= (1 << 11);	//TEST
          }

//						llc.fault_state.bit.over_current = 0 ;	//不过流
          llc.fault_cnt.over_current_cnt = 0;
          llc.short_current_cnt = 0;
        }
    }
...
RAMCODE
void Hw_over_Curr_check_app(void)
{
...
RAMCODE
void fast_short_protect()
{
	if(llc.output_mode == current_mode)
	{
		if(llc.current_mode_cnt > 1)
		{
			hrpwm_outdis_app();
			hrpwm_srdis_app();
			disable_current_sharing();
			llc.state = state_fault;
			llc.current_mode_cnt = 1;
		}
		else
		{
...
{
  if(llc.fault_under_cnt > 2 && !llc.fault_state.bit.over_temp)
    {
      llc.fault_state.bit.under_voltage = 0;
      llc.ocp_time++;
      llc.fault_under_cnt = 0;
    }
  else
    {
      llc.fault_under_cnt ++ ;
		...
		if((!llc.fault_state.all) && (llc.ocp_time <= 6))
    {
      reset_value();
    }
}
