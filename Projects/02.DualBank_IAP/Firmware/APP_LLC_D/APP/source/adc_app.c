...
void adc_init_app(void)
{
  ...
  //User ADC DMA Config
  user_adc_dma_cfg.ch          = IOUT_SAMPLE_CH1;
  ...
  user_adc_cal_cfg.ch       = IOUT_SAMPLE_CH1;
...
//			if(x < 1.885f && x > 1.865f)
//			{
//				llc.vout_trim = 48.0f;
//			}
//			else
			{
				llc.vout_trim = (-15.034f)*x*x+81.6f*x-52.0f;			
			}
    }
...
RAMCODE
float current_cal(float x)
{
    if(x <= 10.0f)
        return x;

    if(x <= 10.3f)  return 10.0f + 1.666667f * (x - 10.0f);
    if(x <= 10.6f)  return 10.5f + 1.666667f * (x - 10.3f);
    if(x <= 11.11f) return 11.0f + 0.980392f * (x - 10.6f);
    if(x <= 11.4f)  return 11.5f + 1.724138f * (x - 11.11f);
    if(x <= 11.8f)  return 12.0f + 1.250000f * (x - 11.4f);
    if(x <= 12.0f)  return 12.5f + 2.500000f * (x - 11.8f);
    if(x <= 12.4f)  return 13.0f + 1.250000f * (x - 12.0f);
    if(x <= 12.8f)  return 13.5f + 1.250000f * (x - 12.4f);

    return 14.0f + 1.250000f * (x - 12.8f);
}
RAMCODE
...
//	llc.iout_rel_slow = current_cal(llc.iout_rel_slow);
