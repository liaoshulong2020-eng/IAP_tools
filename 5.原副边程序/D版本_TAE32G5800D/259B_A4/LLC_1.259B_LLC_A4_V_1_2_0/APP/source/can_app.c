...
bool process_16bit_with_crc(uint8_t cmd, uint16_t* result_value)
{
  ...
  // 验证数据长度（至少需要5字节：保留+命令+2字节数据+CRC）
...
{
  uint8_t current_rx_index = user_can_ctrl.rx_cnt;
  uint8_t byte0 = user_can_ctrl.rx_buf[current_rx_index][start_index];     // 最低字节
  ...
  uint8_t byte3 = user_can_ctrl.rx_buf[current_rx_index][start_index + 3]; // 最高字节
...
bool process_32bit_with_crc(uint8_t cmd, float* result_value, int scale_factor)
{
  ...
  // 验证数据长度（至少需要7字节：保留+命令+4字节数据+CRC）
  ...
  // 提取接收到的数据
  uint8_t received_cmd = user_can_ctrl.rx_buf[current_rx_index][1];
  uint8_t data_byte0 = user_can_ctrl.rx_buf[current_rx_index][2];  // 最低位字节
  uint8_t data_byte1 = user_can_ctrl.rx_buf[current_rx_index][3];
  uint8_t data_byte2 = user_can_ctrl.rx_buf[current_rx_index][4];
  uint8_t data_byte3 = user_can_ctrl.rx_buf[current_rx_index][5];  // 最高位字节
...
void can_send_32bit_simple(uint8_t cmd, uint32_t value)
{
  ...
  uint8_t data_bytes[6] =
  {
    0x00,                             // 保留字节
    cmd,                              // 命令
    (uint8_t)(value & 0xFF),          // byte0 (最低字节)
    (uint8_t)((value >> 8) & 0xFF),   // byte1
    (uint8_t)((value >> 16) & 0xFF),  // byte2
    (uint8_t)((value >> 24) & 0xFF)   // byte3 (最高字节)
  };
...
void process_common_commands(uint8_t can_cmd, uint32_t received_id)
{
    switch(can_cmd) {
        ...
        case CMD_OVERTEMP_POINT:  // 过温点（16位，不带CRC）
        {
            uint16_t rx_data_16;
        ...
        case CMD_OVERTEMP_REC_POINT:  // 过温恢复点（16位，带CRC）
        {
            uint16_t rx_data_16;
        ...
        case CMD_FACTOR_VOLTAGE:  // 校准电压（16位，不带CRC）
        {
            uint16_t rx_data_16;
        ...
        case CMD_THEOR_VOLTAGE:  // 理论电压（16位，不带CRC）
        {
            uint16_t rx_data_16;
        ...
        break;
				...
				case CMD_VOLTAGE_CRC:
				{
						...
						{
								float factor_voltage_float = llc.factor_voltage / 1000.0f; // 当前采样/上位机显示值
								float theor_voltage_float  = llc.theor_voltage  / 1000.0f; // 万用表真实值

								if((factor_voltage_float > 1.0f) && (theor_voltage_float > 1.0f))
								{
										/*
										 * 控制目标校准：
										 * 例如当前目标48.19V，实际应为48.00V
										 * 新目标 = 原目标 / 当前显示值 * 真实值
										 */
										...
										/*
										 * 显示补偿：
										 * 上位机显示值 = llc.vbus_rel - llc.can_com_voltag_delta
										 * 例如 48.19 - 0.19 = 48.00
										 */
...
void LL_CAN_RxCallback(CAN_TypeDef* Instance)
{
    ...
    if(user_can_ctrl.rx_cnt < USER_CAN_RX_FRM_NUMS) {
        ...
        switch(received_id) {
            ...
            default:
                // 检查是否为LLC设备范围的ID（0xA0000-0xA0007）
