...
void load_data_app(void)
{
	...
	llc.protection_point.under_voltage_point			=		 user_data.under_voltage_point;					//欠压保护点
	llc.protection_point.under_voltage_rec_point 	=		 user_data.under_voltage_rec_point;			//欠压恢复点
		
	llc.vout_trim_delta = user_data.vout_trim_delta;
	
	llc.protection_point.over_voltage_point 			=		 user_data.over_voltage_point;					//过压保护点
	llc.protection_point.over_voltage_rec_point 	=		 user_data.over_voltage_rec_point;			//过压恢复点
		
	llc.protection_point.over_current_point 			=		 user_data.over_current_point;					//过流保护点
	llc.protection_point.over_current_rec_point 	=		 user_data.over_current_rec_point;			//过流恢复点	
			
	llc.protection_point.over_temp_point 					=		 user_data.over_temp_point;							//过温保护点
	llc.protection_point.over_temp_rec_point 			=		 user_data.over_temp_rec_point;					//过温恢复点	
	llc.temp_recover_mode 												=		 user_data.temp_recover_mode;						//过温恢复模式
	
	llc.coef_target																=		 user_data.coef_target;
	llc.vbus_target 															=		 llc.coef_target;									//输出目标电压
	llc.can_com_voltag_delta											=		 user_data.vout_can_delta;							//上报通信误差
