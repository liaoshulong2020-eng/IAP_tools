...
void cans_init(ulong id,ulong baudrate)
{
	...
	CAN_AcceptFilCfgTypeDef can_acpt_fil_cfg[3];
	...
	can_acpt_fil_cfg[0].code_val = id;
	can_acpt_fil_cfg[0].mask_val = 0;
	can_acpt_fil_cfg[0].rx_frm = CAN_ACCEPT_FILT_FRM_STD_EXT;
	can_acpt_fil_cfg[1] = can_acpt_fil_cfg[0];
	can_acpt_fil_cfg[1].slot = CAN_ACCEPT_FILT_SLOT_1;
	can_acpt_fil_cfg[1].code_val = IAP_LEGACY_CAN_ID;
	can_acpt_fil_cfg[2] = can_acpt_fil_cfg[0];
	can_acpt_fil_cfg[2].slot = CAN_ACCEPT_FILT_SLOT_2;
	can_acpt_fil_cfg[2].code_val = IAP_DISCOVERY_CAN_ID;
...
void cans_send_data(const void *data,uchar size)
{
	...
	tx_buf_fmt.id = __LL_CAN_FrameIDFormat_29Bits(can_id);
