...
void llc_PI_vloop(void)
{
  ...
  const float ERROR_HIGH = 3.0f;
  const float KP_LOW = 80.0f, KP_HIGH = 420.0f;
  const float KI_LOW = 5.0f, KI_HIGH = 15.0f;
  ...
  if(llc.state == state_rampup)
    {
			...
			{
			  llc.vloop.kp = 300.0f;//llc.vloop_kp_init;//100.0f;
				llc.vloop.ki = 40.0f;//llc.vloop_ki_init;//5.0f;
			}
			else
			{
			  llc.vloop.kp = 200.0f;
