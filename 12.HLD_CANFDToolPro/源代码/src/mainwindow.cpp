    ...
    , sendThread_{ sendThread(&device_), sendThread(&device_) }
{
    ...
    setWindowTitle(QStringLiteral("HLD_CANFDToolPro v1.0"));
    ...
    appendLog(QStringLiteral("HLD_CANFDToolPro v1.0 启动。"));
}
...
QWidget *MainWindow::makeIapPage()
{
    ...
    mode->addItems({ tr("节点ID 0xA0000~0xA0007"), tr("固定ID 0xAA55") });
    ...
    grid->addWidget(new QLabel(tr("固件:"), settings), 3, 0);
    ...
    connect(mode, &QComboBox::currentIndexChanged, this, [nodes, start](int index) {
        nodes->setVisible(index == 0);
        start->setText(index == 0 ? tr("批量升级") : tr("固定ID升级"));
    });

    connect(start, &QPushButton::clicked, this, [this, file, log, state, progress, start, stop, target, channel, mode, nodeChecks] {
        if (file->text().isEmpty()) { QMessageBox::warning(this, tr("固件升级"), tr("请先选择 BIN 或 HEX 固件文件。")); return; }
        auto ids = std::make_shared<QList<uint32_t>>();
        if (mode->currentIndex() == 1) *ids << 0xAA55U;
        else for (int i = 0; i < nodeChecks.size(); ++i) if (nodeChecks[i]->isChecked()) *ids << 0xA0000U + uint32_t(i);
